...
class DatabaseHelper {
  ...
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 3) {
      for (String table in tables) {
        // SQLite doesn't support IF NOT EXISTS for columns natively, wrapping in try-catch for safety
        try { 
          await db.execute('ALTER TABLE $table ADD COLUMN parentId TEXT'); 
        } catch (_) {}
        
        try { 
          await db.execute('ALTER TABLE $table ADD COLUMN updatedAt INTEGER DEFAULT 0'); 
        } catch (_) {}
        
        try { 
          await db.execute('ALTER TABLE $table ADD COLUMN isDeleted INTEGER DEFAULT 0'); 
        } catch (_) {}
      }
    }
  }
  ...
  // ==========================================
  // INTERCEPTED CRUD OPERATIONS (Soft Deletes & Auto-Sync)
  // ==========================================
  ...
  // ==========================================
  // READ OPERATIONS (Filtered by isDeleted = 0)
  // ==========================================
  ...
  Future<void> mergeDatabaseJSON(String remoteJson) async {
    final db = await instance.database;
