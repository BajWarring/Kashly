...
class _CashbookScreenState extends State<CashbookScreen> {
  ...
  void initState() {
    super.initState();
    _loadEntries();
    // Rebuild entry borders whenever sync state changes.
  ...
  Future<void> _loadEntries() async {
    ...
    final data =
        await DatabaseHelper.instance.getEntriesForBook(widget.book.id);

    double running = 0;
    double tIn = 0;
    double tOut = 0;
    final Map<String, double> bals = {};
  ...
  void _openFilter(String type) {
    if (type == 'type') {
      FilterDialogs.showSelectionDialog(context, 'Entry Type',
              ['All Entries', 'Cash In', 'Cash Out'], false)
          .then((res) {
      ...
      DatabaseHelper.instance.getAllOptions('Category').then((opts) {
        if (!mounted) return;
        FilterDialogs.showSelectionDialog(
                context, 'Categories', opts.map((e) => e.value).toList(), true)
            .then((res) {
          if (res != null && mounted) {
      ...
      DatabaseHelper.instance.getAllOptions('Payment Method').then((opts) {
        if (!mounted) return;
        FilterDialogs.showSelectionDialog(context, 'Payment Method',
                opts.map((e) => e.value).toList(), true)
            .then((res) {
          if (res != null && mounted) {
  ...
  PreferredSizeWidget _buildAppBar() {
    final filterActive = _filterType != 'All Entries' ||
        _filterCategories.isNotEmpty ||
        _filterPayments.isNotEmpty;
        ...
        actions: [
          PopupMenuButton<String>(
            icon: Icon(Icons.filter_list,
                color: filterActive ? accent : textMuted),
  ...
  void _openAddEntryScreen(String type) async {
    await Navigator.push(context,
        MaterialPageRoute(
            builder: (_) =>
                AddEntryScreen(book: widget.book, initialType: type)));
    _loadEntries();
  }

  // ── Entry row ──────────────────────────────────────────────────────────────

  Widget _buildEntryRow(Entry entry, bool showDateHeader,
      String dateStr, String timeStr, bool isUnsynced) {
        ...
        AnimatedContainer(
          ...
          child: InkWell(
            ...
            onTap: () => Navigator.push(
              ...
              MaterialPageRoute(
                  builder: (_) =>
                      EntryDetailsScreen(entry: entry, book: widget.book)),
            ).then((_) => _loadEntries()),
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Container(
                    ...
                    height: 36,
                    decoration: BoxDecoration(
                        color: eBg, borderRadius: BorderRadius.circular(12)),
                    child: Icon(
                  ...
                  // Amount + balance + unsynced badge
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      Text(
                        (isIn ? '+' : '-') + _formatCur(entry.amount),
                        style: TextStyle(
                            fontSize: 14,
                          ...
                          child: Row(
                            ...
                            children: [
                              Icon(Icons.cloud_upload_rounded,
                                  size: 10,
                                  color: warning.withValues(alpha: 0.9)),
  ...
  Widget build(BuildContext context) {
    final displayEntries = entries.where((e) {
      ...
      bool typeMatch = _filterType == 'All Entries' ||
          (_filterType == 'Cash In' && e.type == 'in') ||
          (_filterType == 'Cash Out' && e.type == 'out');
      bool catMatch = _filterCategories.isEmpty ||
          _filterCategories.contains(e.category);
      bool payMatch = _filterPayments.isEmpty ||
          _filterPayments.contains(e.paymentMethod);
              ...
              children: [
                    ...
                    children: [
                      const Text('OVERALL BALANCE',
                          style: TextStyle(
                              ...
                              letterSpacing: 1)),
                      const SizedBox(height: 2),
                      Text(_formatCur(widget.book.balance),
                          style: const TextStyle(
                              color: textDark,
                              fontSize: 28,
                              fontWeight: FontWeight.w900)),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () => Navigator.push(
                              ...
                              MaterialPageRoute(
                                  builder: (_) =>
                                      GenerateReportScreen(book: widget.book))),
                ...
                Padding(
                  ...
                  child: Row(
                      ...
                      children: [
                        Text('Showing ${displayEntries.length} Entries',
                            style: const TextStyle(
                                fontSize: 12,
                                fontWeight: FontWeight.bold,
                                color: textMuted))
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      if (index > 0) {
                        final prevDate = DateFormat('MMM d, yyyy').format(
                            DateTime.fromMillisecondsSinceEpoch(
                                displayEntries[index - 1].timestamp));
                        if (prevDate == dateStr) showDateHeader = false;
                      }

                      // Unsynced when signed in and entry was written after last sync
                      final bool isUnsynced = sync.isSignedIn &&
                          entry.updatedAt > sync.lastSyncTime;

                      return _buildEntryRow(
                          entry, showDateHeader, dateStr, timeStr, isUnsynced);
                    },
                  ),
                ),
              ],
