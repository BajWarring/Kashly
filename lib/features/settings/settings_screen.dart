...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
        ...
        children: [
          ...
          ListTile(
            ...
            title: Text(authState.isAuthenticated
                ...
                : 'Sign In with Google'),
            subtitle: Text(authState.isAuthenticated ? 'Drive backup enabled' : 'Not connected'),
            trailing: authState.isAuthenticated
                ? PopupMenuButton<String>(
                    onSelected: (v) {
                      if (v == 'switch') ref.read(authProvider.notifier).switchAccount();
            ...
            onTap: authState.isAuthenticated ? null : () => context.go('/auth'),
          ),
          const Divider(),
          ...
          backupSettingsAsync.when(
            data: (settings) => _BackupSettingsSection(
              settings: settings,
              onSave: (s) => ref.read(backupRepositoryProvider).saveSettings(s).then((_) => ref.invalidate(backupSettingsProvider)),
            ),
            ...
            data: (history) => Column(
              children: [
                ListTile(
                  ...
                  onTap: () async {
                    try {
                      final file = await generateBackupReportPdf(history);
                ...
                ListTile(
                  ...
                  onTap: () async {
                    try {
                      final file = await exportBackupManifest(history);
          ...
          ListTile(
            ...
            subtitle: const Text('Backup success, failures, pending sync'),
            trailing: const Icon(Icons.chevron_right),
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (mounted) showSuccessSnackbar(context, 'Resync triggered');
                },
              ),
              ListTile(
                leading: const Icon(Icons.bug_report_outlined),
                title: const Text('Export Debug Logs'),
                onTap: () => showSuccessSnackbar(context, 'Debug log exported'),
              ),
            ],
          ),
          const Divider(),
          ...
          const ListTile(
            ...
            title: Text('Kashly'),
            subtitle: Text('Version 1.0.0 · Professional Cashbook Manager'),
          ),
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            SwitchListTile(title: const Text('Backup Success'), value: true, onChanged: (_) {}),
            SwitchListTile(title: const Text('Backup Failure'), value: true, onChanged: (_) {}),
            SwitchListTile(title: const Text('Pending Sync Reminder'), value: true, onChanged: (_) {}),
            SwitchListTile(title: const Text('Conflict Detected'), value: true, onChanged: (_) {}),
          ],
        ),
  ...
  Future<void> _vacuumDb(BuildContext context) async {
    try {
      ...
      if (mounted) showSuccessSnackbar(context, 'Database optimized successfully');
    } catch (e) {
  ...
  Future<void> _clearCache(BuildContext context) async {
    final confirmed = await showDeleteConfirmation(context, 'local cache');
    if (confirmed == true) {
...
class _BackupSettingsSection extends StatefulWidget {
  ...
  const _BackupSettingsSection({required this.settings, required this.onSave});
    ...
    return Column(
      children: [
        ...
        Card(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('Backup Summary', style: TextStyle(fontWeight: FontWeight.w600)),
                const SizedBox(height: 8),
                const Row(children: [Icon(Icons.storage_outlined, size: 16), SizedBox(width: 8), Text('Last Local: N/A', style: TextStyle(fontSize: 13))]),
                const Row(children: [Icon(Icons.cloud_outlined, size: 16), SizedBox(width: 8), Text('Last Drive: N/A', style: TextStyle(fontSize: 13))]),
                Row(children: [
                  const Icon(Icons.schedule_outlined, size: 16),
                  const SizedBox(width: 8),
                  Text('Interval: ${_settings.autoBackupInterval.name}', style: const TextStyle(fontSize: 13)),
                ]),
              ],
            ),
          ),
        ...
        SwitchListTile(
          ...
          subtitle: const Text('Automatically sync to Google Drive'),
          value: _settings.autoBackupEnabled,
          onChanged: (v) => _update(_settings.copyWith(autoBackupEnabled: v)),
        ),
        if (_settings.autoBackupEnabled) ...[
          ListTile(
            ...
            trailing: DropdownButton<AutoBackupInterval>(
              ...
              onChanged: (v) => _update(_settings.copyWith(autoBackupInterval: v)),
              items: AutoBackupInterval.values
                  .map((i) => DropdownMenuItem(value: i, child: Text(i.name.capitalize())))
                  .toList(),
            ),
          ),
          SwitchListTile(
            ...
            onChanged: (v) => _update(_settings.copyWith(onlyOnUnmeteredNetwork: v)),
          ),
          SwitchListTile(
            ...
            onChanged: (v) => _update(_settings.copyWith(backupOnlyWhenCharging: v)),
          ),
        ],
        SwitchListTile(
          ...
          onChanged: (v) => _update(_settings.copyWith(includeAttachmentsInDrive: v)),
        ),
        SwitchListTile(
          secondary: const Icon(Icons.lock_outline),
          title: const Text('Encrypt Backups'),
          value: _settings.encryptionEnabled,
          onChanged: (v) => _update(_settings.copyWith(encryptionEnabled: v)),
        ),
        SwitchListTile(
          ...
          onChanged: (v) => _update(_settings.copyWith(driveAutoVersioning: v)),
        ),
      ],
    );
  }
